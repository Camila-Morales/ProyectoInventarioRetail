...
function hasNoneOrValidComponentName(context, node) {
...
function useComponentCollector(context, options = {}) {
  ...
  const listeners = {
    ...
    "ArrowFunctionExpression[type][body.type!='BlockStatement']"() {
      ...
      const isComponent = hasNoneOrValidComponentName(context, entry.node) && JSX3.isJSXValue(body, jsxCtx, hint) && hasValidHierarchy(context, entry.node, hint);
    ...
    "ReturnStatement[type]"(node) {
      const entry = getCurrentEntry();
      if (entry == null) return;
      const isComponent = hasNoneOrValidComponentName(context, entry.node) && JSX3.isJSXValue(node.argument, jsxCtx, hint) && hasValidHierarchy(context, entry.node, hint);
      if (!isComponent) return;
