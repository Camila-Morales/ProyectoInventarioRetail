...
function getName(node) {
  if (AST11.isTsOnlyExpression(node)) {
    ...
    return {
      ...
      VariableDeclarator(node) {
        ...
        const hasState = node.id.properties.some(
          (prop) => prop.type === AST_NODE_TYPES.Property && isKeyLiteral(prop, prop.key) && getName(prop.key) === "state"
        );
        if (!hasState) {
          return;
        }
...
var no_context_provider_default = createRule({
  ...
  create(context) {
    ...
    return {
      JSXElement(node) {
        const [name5, ...rest] = JSX8.getElementName(node).split(".").reverse();
...
function isAssignmentToThisState(node) {
  const { left } = node;
  return left.type === AST_NODE_TYPES.MemberExpression && AST11.isThisExpression(left.object) && getName2(left.property) === "state";
}
...
var no_unstable_context_value_default = createRule({
  ...
  create(context) {
    ...
    return {
      ...listeners,
      JSXOpeningElement(node) {
        const fullName = JSX8.getElementName(node.parent);
...
function isAssignmentToThisState2(node) {
  const { left } = node;
  return left.type === AST_NODE_TYPES.MemberExpression && AST11.isThisExpression(left.object) && getName4(left.property) === "state";
}
    ...
    return {
      ...
      "Program:exit"(node) {
        ...
        function isFunctionComponent(block) {
          ...
          return id != null && isComponentName(id.name) && components.some((component) => component.node === block);
        }
